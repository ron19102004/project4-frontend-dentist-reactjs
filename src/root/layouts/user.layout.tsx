...
import RouteItem, {INavRoute, loginRoute, navLeftRoute, navRightRoute} from "@/components/nav/user/index.tsx";
import {ChatBot, GoogleMap, LoadingLine} from "@/components/ui";
import {useAuth, useBoolean, useScreen} from "@/hooks";
...
export default function Userlayout() {
    ...
    const accountRoute = (role: string): INavRoute => {
        switch (role) {
            case "ACCOUNTANT":
            case "DENTIST":
            case "ADMIN": {
        ...
        <section className="min-w-screen min-h-screen transition-all">
            ...
            <div className={cn('fixed right-5 md:right-10 bottom-5 md:bottom-10 z-50', {
                ...
                <button onClick={onTop}>
                    <img src={assets.UpIcon} alt="up"
                         className="w-12 h-12 md:w-14 md:h-14 rounded-full p-2 shadow border bg-white"/>
                </button>
            </div>
            <header>
                ...
                <section className={cn({
                    ...
                    {/* FOR MOBILE  */}
                    {isMobile &&
                        <div className="lg:hidden flex justify-between items-center px-4 py-2 bg-white border-b">
                            {/* LOGO MOBILE */}
                    ...
                    <nav
                        className={cn('bg-white border-b px-4 py-4 flex flex-col lg:flex-row justify-around lg:items-center lg:px-0 gap-6', {
                        ...
                        <ul className='flex flex-col lg:flex-row justify-around lg:items-center gap-6'>
                            ...
                            />
                            {!isAuthenticated ? <RouteItem route={loginRoute}/> :
                                <RouteItem route={accountRoute(role)}/>}
                        </ul>

                        {/* LOGO ICONIC  */}
                        <section
                            className={cn("hidden absolute bg-white lg:flex flex-col justify-center items-center left-[50%] -translate-x-[50%] shadow-2xl z-30 border", {
