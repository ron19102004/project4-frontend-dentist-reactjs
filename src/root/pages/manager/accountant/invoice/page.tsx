...
const AccountantInvoicePage = () => {
    const {accessToken} = useAuth()!
    const {getInvoicesByStatusAndPage} = useInvoice(accessToken)
    const [list, setList] = useState<InvoiceForAccountant[]>([]);
    ...
    const handlePageChange = async (type: "plus" | "minus") => {
        if (pageNumber > 0 ) {
            ...
            await initialize(pageNumber, invoiceStatus)
        }
    };
